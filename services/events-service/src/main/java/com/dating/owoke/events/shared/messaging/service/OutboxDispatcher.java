...
public class OutboxDispatcher {
    ...
    public OutboxDispatcher(OutboxEventRepository repository, KafkaTemplate<String, String> kafkaTemplate, Clock clock) {
        this.repository = repository; this.kafkaTemplate = kafkaTemplate; this.clock = clock;
    }
    @Scheduled(fixedDelayString = "${owoke.outbox.fixed-delay:1000}")
    @Transactional
    public void publishPending() {
        for (var event : repository.lockPending(clock.instant(), PageRequest.of(0, 50))) {
            try {
